...
//        중복 체크 (금융회사 + 상품명)
    ...
    public List<PersonalCreditLoanDTO> getPersonalCreditLoansByFinancialCompanyId(Long financialCompanyId) {
        ...
        List<PersonalCreditLoan> entityList = personalCreditLoanMapper.selectPersonalCreditLoansByFinancialCompanyId(financialCompanyId);
        return entityList.stream()
                .map(PersonalCreditLoanDTO::fromEntity)
                .toList();
    }
    ...
    public List<PersonalCreditLoanDTO> getPersonalCreditLoansByCrdtPrdtTypeNm(String crdtPrdtTypeNm) {
        ...
        List<PersonalCreditLoan> entityList = personalCreditLoanMapper.selectPersonalCreditLoansByCrdtPrdtTypeNm(crdtPrdtTypeNm);
        return entityList.stream()
                .map(PersonalCreditLoanDTO::fromEntity)
                .toList();
    }
        ...
        try {
            ...
            for (JsonNode baseNode : baseList) {
                ...
                if (matchedOption != null) {
                    PersonalCreditLoan entity = createPersonalCreditLoanEntity(baseNode, matchedOption, financialCompanyId);
                    if (entity != null) {
                        entities.add(entity);
                    }
                }
...
//    실제로는 financial_company 테이블을 조회하여 매핑해야 함
    ...
    private PersonalCreditLoan createPersonalCreditLoanEntity(JsonNode baseNode, JsonNode optionNode, Long financialCompanyId) {
        try {
