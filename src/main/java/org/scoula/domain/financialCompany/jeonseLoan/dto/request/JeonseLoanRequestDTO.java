package org.scoula.domain.financialCompany.jeonseLoan.dto.request;
...
@AllArgsConstructor
@Builder
@ApiModel(description = "전세자금대출 등록 요청 DTO")
public class JeonseLoanRequestDTO {
    ...
    public JeonseLoan toEntity() {
        return JeonseLoan.builder()
                ...
                .name(name)
                .joinWay(joinWay)
                .loanExpensive(loanExpensive)
                .erlyFee(erlyFee)
                .dlyRate(dlyRate)
                .loanLmt(loanLmt)
                .jeonseId(jeonseId)
                .repayTypeName(repayTypeName)
                .lendRateTypeName(lendRateTypeName)
                .lendRateMin(lendRateMin)
                .lendRateMax(lendRateMax)
                .lendRateAvg(lendRateAvg)
                .build();
    }
}
